...
@app.route('/process_singleprocess', methods=['POST'])
def process_request_singleprocess():
...
def process_request_multiprocess(data):
    print("Start ")
    youtube_url = data['youtube_url']
    resolution = data['resolution']
    image_path = data['image_path']
...
@app.route('/process_request_multiprocess_endpoint', methods=['POST'])
def process_request_multiprocess_endpoint():
    data = request.get_json()

    # ToDO: We can verify the data params are correct here
    youtube_url = data['youtube_url']
    resolution = data['resolution']
    image_path = data['image_path']
    chunk_duration = data['chunk_duration']
    ...
    with multiprocessing.Pool() as pool:
        result = pool.apply_async(process_request_multiprocess, (data,))
        hls_stream_url = result.get()
